    ...
    <div style={{ padding: "16px" }}>
      ...
      {qs.map((q, i) => {
        ...
        return (
          <div key={i} style={{
            background: t.bgCard,
            border: `1px solid ${isSkipped ? t.border : isRight ? t.correctBorder : t.wrongBorder}`,
            ...
            {!isRight && (
              <div style={{
                fontSize: 12, color: t.expText,
                background: t.expBg, borderRadius: 8,
...
export default function App() {
  ...
  // Hardware back button — fixed stale closure with ref
  ...
  if (screen === "feedback") {
    return (
      <Feedback
        ...
        onSend={sendFeedback} onBack={goBack}
        goldBtn={goldBtn} card={card}
      />
    );
  }
  ...
  if (screen === "notes" && data) {
    return (
      <NotesCourses t={t} data={data} activeSubject={activeSubject} onBack={goBack}
        onSelectCourse={(c) => { setNoteCourse(c); goTo("notes_topics"); }} />
    );
  }
  if (screen === "notes_topics" && noteCourse && data) {
    return (
      <NotesTopics t={t} data={data} noteCourse={noteCourse} onBack={goBack}
  ...
  if (screen === "pastq_courses" && data) {
    return (
      <PastQCourses t={t} data={data} activeSubject={activeSubject} onBack={goBack}
        onSelectCourse={(c) => { setPqCourse(c); goTo("pastq_topics"); }} />
    );
  }
  if (screen === "pastq_topics" && pqCourse && data) {
    return (
      <PastQTopics t={t} data={data} pqCourse={pqCourse} onBack={goBack}
    ...
    if (cbtDone) {
      ...
      return (
        <div style={wrap}>
          ...
          {/* ✅ FIX: mode and activeSubject now passed */}
          <ResultScreen
            qs={cbtQs} answers={cbtAnswers} t={t}
            mode="CBT" activeSubject={activeSubject}
    ...
    if (examDone) {
      ...
      return (
        <div style={wrap}>
          ...
          {/* ✅ FIX: mode and activeSubject now passed */}
          <ResultScreen
            qs={examQs} answers={examAnswers} t={t}
            mode="Exam" activeSubject={activeSubject}
