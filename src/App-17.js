  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: "28px 24px", width: 290,
        textAlign: "center",
        border: `1px solid ${t.border}`,
        boxShadow: "0 12px 40px rgba(0,0,0,0.4)",
      }}>
        <div style={{ fontSize: 36, marginBottom: 12 }}>⚠️</div>
        <div style={{ fontSize: 16, fontWeight: "bold", color: t.heading, marginBottom: 8 }}>
          Quit Quiz?
        </div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button
            ...
            style={{
              flex: 1, padding: "13px 0", borderRadius: 10,
              background: t.bgInner, color: t.textSub,
              border: `1px solid ${t.border}`,
...
function ResultScreen({ qs, answers, t, onRetry, onHome, activeSubject, mode }) {
  ...
  const gradeShort =
    pct >= 70 ? "Grade A" : pct >= 60 ? "Grade B" :
    pct >= 50 ? "Grade C" : pct >= 45 ? "Grade D" :
...
function SubjectSelect({ t, onToggleTheme, onBack, onSelect, mode }) {
  ...
  const label = modeLabels[mode] || { title: "Select", sub: "" };
      ...
      <div style={{ padding: "20px 16px" }}>
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              {subject.available
                ...
                : <div style={{ fontSize: 12, marginTop: 4 }}><span style={{ background: "#f59e0b", color: "#000", fontSize: 10, fontWeight: "bold", padding: "2px 8px", borderRadius: 20 }}>COMING SOON</span></div>
              }
            </div>
            {subject.available
        ...
        <div style={{ background: t.keyBg, border: `1px solid ${t.keyBorder}`, borderRadius: 12, padding: "14px 16px", marginTop: 8 }}>
          <div style={{ fontSize: 13, color: t.keyText, lineHeight: 1.8 }}>
            🔑 More subjects coming soon. Currently available: <strong>Economics, Government</strong>.
          </div>
        </div>
      </div>
    </div>
...
function StreakBanner({ streak, t }) {
  ...
  return (
    <div style={{ background: t.keyBg, border: `1px solid ${tier.color}66`, borderRadius: 12, padding: "12px 16px", marginBottom: 14, display: "flex", alignItems: "center", gap: 12 }}>
      <div style={{ width: 44, height: 44, borderRadius: 12, background: `${tier.color}22`, border: `1px solid ${tier.color}55`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 }}>
        {tier.icon}
      </div>
      <div style={{ flex: 1 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
          <span style={{ fontSize: 15, fontWeight: "bold", color: tier.color }}>{streak.count}-day streak</span>
          <span style={{ fontSize: 9, fontWeight: "bold", letterSpacing: 1, color: tier.color, background: `${tier.color}22`, border: `1px solid ${tier.color}44`, borderRadius: 20, padding: "2px 7px", textTransform: "uppercase" }}>{tier.label}</span>
        </div>
        <div style={{ fontSize: 11, color: t.keyText, opacity: 0.85 }}>{tier.msg}</div>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  // 1. Toast — must come first so showToast is ready for effects below
  ...
  // 7. Notifications
  const {
    permission, settings: notifSettings,
    loading: notifLoading, error: notifError,
    requestPermission, disableNotifications,
    updateSettings: updateNotifSettings,
  } = useNotifications();

  // ── 3-MINUTE WARNINGS ─────────────────────────────────────────────────────
  // These live in App.js where showToast is guaranteed to exist.
  // useQuiz timers are pure — they only update state.
  // These effects watch that state and fire the toast safely.
  ...
  useEffect(() => {
    if (cbtTime === 3600) {
      cbtWarnedRef.current = false; // reset when new CBT starts
    }
  ...
  useEffect(() => {
    if (examTime > 200) {
      examWarnedRef.current = false; // reset when new exam starts
    }
  ...
  if (showSplash || authLoading) {
    ...
    return (
      <div style={{ minHeight: "100vh", background: isDark ? "linear-gradient(135deg,#0a1a0a 0%,#1a3a1a 50%,#0d2b0d 100%)" : "linear-gradient(135deg,#f0f4f8 0%,#e8f0e8 50%,#f5f0e8 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "Georgia, serif", padding: "40px 20px" }}>
        ...
        <div style={{ width: "60%", maxWidth: 200, height: 4, background: isDark ? "#1e2e1e" : "#ddd8cc", borderRadius: 10, overflow: "hidden", marginBottom: 16 }}>
          <div style={{ height: "100%", width: authLoading ? "70%" : `${progress}%`, background: isDark ? "linear-gradient(90deg,#c8a84b,#f0d080)" : "linear-gradient(90deg,#1a3a5c,#2563eb)", borderRadius: 10, transition: "width 0.05s linear" }} />
        </div>
        <div style={{ fontSize: 11, color: isDark ? "#666" : "#888", letterSpacing: 1 }}>
          {authLoading ? "Restoring your account... 👤" : progress < 40 ? "Loading questions..." : progress < 70 ? "Preparing study materials..." : progress < 90 ? "Almost ready..." : "Welcome! 🥳"}
        </div>
  ...
  if (screen === "home") {
    ...
    return (
      <div style={wrap}>
        {/*
          ┌─────────────────────────────────────────┐
          │  Toast sits here — always on top,        │
          │  visible across the entire home screen   │
          └─────────────────────────────────────────┘
        */}
        ...
        <div style={{ padding: "16px" }}>
          ...
          <div style={{ background: t.heroBg, borderRadius: 16, padding: "18px 16px", marginBottom: 20, border: `1px solid ${t.heroBorder}` }}>
            ...
            <div style={{ fontSize: 12, color: t.gold, marginTop: 6, fontWeight: "bold" }}>📚 {totalQ}+ questions available</div>
  ...
  if (screen === "subject_select") {
    return <SubjectSelect t={t} onToggleTheme={toggleTheme} onBack={goBack} mode={pendingMode}
      onSelect={(subject) => {
      ...
      <div style={wrap}>
        ...
        <div style={{ padding: "16px" }}>
          <div style={{ margin: 30, padding: "0 10px" }}>
            <p style={{ fontSize: 16, color: t.textSub, textAlign: "center", lineHeight: "18px" }}>© 2026 StudyNaija. All rights reserved.</p>
            <p style={{ fontSize: 15, color: t.textSub, textAlign: "center", marginTop: 6 }}>StudyNaija is an independent educational platform and is not affiliated with, endorsed by, or associated with <strong>JUPEB</strong> or any official examination body.</p>
            <p style={{ fontSize: 15, color: t.textSub, textAlign: "center", marginTop: 6 }}>All trademarks and names belong to their respective owners.</p>
          </div>
        </div>
      </div>
    );
  ...
  if (screen === "notes" && data)               return <NotesCourses t={t} data={data} activeSubject={activeSubject} onBack={goBack} onSelectCourse={(c) => { setNoteCourse(c); goTo("notes_topics"); }} />;
  if (screen === "notes_topics" && noteCourse)  return <NotesTopics  t={t} data={data} noteCourse={noteCourse}       onBack={goBack} onSelectTopic={(tp) => { setNoteTopic(tp); goTo("notes_view"); }} />;
  if (screen === "notes_view"   && noteTopic)   return <NotesView    t={t} data={data} noteTopic={noteTopic}         onBack={goBack} card={card} />;

  if (screen === "pastq_courses" && data)       return <PastQCourses t={t} data={data} activeSubject={activeSubject} onBack={goBack} onSelectCourse={(c) => { setPqCourse(c); goTo("pastq_topics"); }} />;
  if (screen === "pastq_topics"  && pqCourse)   return <PastQTopics  t={t} data={data} pqCourse={pqCourse}           onBack={goBack} onSelectTopic={(tp) => { setPqTopic(tp); goTo("pastq_view"); }} />;
  if (screen === "pastq_view"    && pqTopic)    return <PastQView    t={t} data={data} pqTopic={pqTopic}             onBack={goBack} card={card} />;
    ...
    if (cbtDone) {
      ...
      return (
        <div style={wrap}>
          ...
          <ResultScreen qs={cbtQs} answers={cbtAnswers} t={t} mode="CBT" activeSubject={activeSubject} onRetry={() => startCbt(activeSubject, goTo, user?.uid)} onHome={() => goTo("home")} />
        </div>
      );
    }
    return (
      <>
        {/*
          ┌─────────────────────────────────────────────┐
          │  Toast sits here — floats above CbtQuiz     │
          │  This is where the 3-min warning appears    │
          └─────────────────────────────────────────────┘
        */}
        <Toast toast={toast} onHide={hideToast} />
        ...
        <CbtQuiz
          ...
          onBack={() => {
            if (Object.keys(cbtAnswers).length > 0) {
              setQuitModal({ open: true, onConfirm: () => { setCbtRunning(false); setQuitModal({ open: false, onConfirm: null }); goTo("home"); } });
            } else { goTo("home"); }
          }}
  ...
  if (screen === "exam_setup" && data) {
    return <ExamSetup t={t} data={data} activeSubject={activeSubject} firebaseQuestions={firebaseQuestions} selectedYear={selectedYear} examCount={examCount} setExamCount={setExamCount} examMinutes={examMinutes} setExamMinutes={setExamMinutes} onStart={() => startExam(activeSubject, goTo, selectedYear, user?.uid)} onBack={goBack} goldBtn={goldBtn} card={card} />;
  }
    ...
    if (examDone) {
      ...
      return (
        <div style={wrap}>
          ...
          <ResultScreen qs={examQs} answers={examAnswers} t={t} mode="Exam" activeSubject={activeSubject} onRetry={() => startExam(activeSubject, goTo, selectedYear, user?.uid)} onHome={() => goTo("home")} />
        </div>
      );
    }
    return (
      <>
        {/*
          ┌─────────────────────────────────────────────┐
          │  Toast sits here — floats above ExamQuiz    │
          │  This is where the 3-min warning appears    │
          └─────────────────────────────────────────────┘
        */}
        <Toast toast={toast} onHide={hideToast} />
        ...
        <ExamQuiz
          ...
          onBack={() => {
            if (Object.keys(examAnswers).length > 0) {
              setQuitModal({ open: true, onConfirm: () => { setExamRunning(false); setQuitModal({ open: false, onConfirm: null }); goTo("exam_setup"); } });
            } else { goTo("exam_setup"); }
          }}
          card={card} goldBtn={goldBtn}
        />
        <QuitModal open={quitModal.open} onConfirm={quitModal.onConfirm} onCancel={() => setQuitModal({ open: false, onConfirm: null })} t={t} />
