...
function getMergedQuestions(data, firebaseQuestions, subjectId, topicId) {
  const local = data.questions[topicId] || [];

  const fbFiltered = (firebaseQuestions || [])
  ...
  return [...local, ...fbFiltered];
}

// ── Helper: total questions for a course (local + Firebase) ───────────────────
function getCourseTotal(data, firebaseQuestions, subjectId, course) {
  return course.topics.reduce((sum, tp) => {
    return sum + getMergedQuestions(data, firebaseQuestions, subjectId, tp.id).length;
  }, 0);
}
      ...
      <div style={{ padding: "16px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {data.courses.map((c, ci) => {
            const qCount = getCourseTotal(data, firebaseQuestions, activeSubject?.id, c);
            return (
...
// ── PAST Q TOPICS ─────────────────────────────────────────────────────────────
export function PastQTopics({ t, data, pqCourse, activeSubject, firebaseQuestions, onSelectTopic, onBack }) {
  return (
    <ScreenWrapper>
      <div style={{ background: t.bgHeader, borderBottom: `2px solid ${t.gold}`, padding: "14px 16px", display: "flex", alignItems: "center", gap: 10, position: "sticky", top: 0, zIndex: 100 }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: "#fff", fontSize: 22, cursor: "pointer", padding: 0 }}>←</button>
      ...
      <div style={{ padding: "16px" }}>
        {pqCourse.topics.map((tp, i) => {
          const allQs = getMergedQuestions(data, firebaseQuestions, activeSubject?.id, tp.id);
...
// ── PAST Q VIEW ───────────────────────────────────────────────────────────────
export function PastQView({ t, data, pqTopic, activeSubject, firebaseQuestions, onBack, card }) {
  const [revealed, setRevealed] = useState({});
  const [selected, setSelected] = useState({});

  // Merge local + Firebase questions for this topic
  const pqs = getMergedQuestions(data, firebaseQuestions, activeSubject?.id, pqTopic.id);
          ...
          return (
            <div key={qi}>
              <div style={{ background: t.bgCard, border: `1px solid ${isRev ? t.correctBorder : t.border}`, borderRadius: 16, padding: "18px 16px", marginBottom: 14 }}>
                ...
                {/* Year tag */}
                <div style={{ fontSize: 10, color: t.tagColor, letterSpacing: 2, marginBottom: 10 }}>
                  JUPEB {q.year}
                  {q.fromFirebase && (
                    <span style={{ marginLeft: 8, fontSize: 9, background: t.exBg, color: t.exText, borderRadius: 10, padding: "1px 7px", border: `1px solid ${t.exBorder}` }}>
                      ✦ New
                    </span>
                  )}
                </div>
...
PastQCourses.propTypes = {
  ...
  activeSubject: PropTypes.shape({ id: PropTypes.string, name: PropTypes.string }),
...
PastQTopics.propTypes = {
  t: PropTypes.object.isRequired,
  data: PropTypes.object.isRequired,
  pqCourse: PropTypes.shape({ title: PropTypes.string, code: PropTypes.string, topics: PropTypes.array }).isRequired,
  activeSubject: PropTypes.shape({ id: PropTypes.string }),
  firebaseQuestions: PropTypes.array.isRequired,
...
PastQView.propTypes = {
  t: PropTypes.object.isRequired,
  data: PropTypes.object.isRequired,
  pqTopic: PropTypes.shape({ id: PropTypes.string, label: PropTypes.string }).isRequired,
  activeSubject: PropTypes.shape({ id: PropTypes.string }),
  firebaseQuestions: PropTypes.array.isRequired,
  onBack: PropTypes.func.isRequired,
